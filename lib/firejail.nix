let
  script = { relativePath, firejailBinary, firejailArgs, ... }: ''
    (
  ...
  wrapPhase = { package, makeWrapper, lib, ... }@args:
    let
    ...
    in package.overrideAttrs (old:
      {
        ...
        doInstallCheck = lib.warnIf (old.doInstallCheck or false)
          "disabeling installCheck for `${
            lib.getName package
          }` because firejail breaks it" false;
      } // (if (old.phases or [ ]) != [ ] then {
        phases = ensureElem old.phases "wrapFirejailPhase";
      } else
        { }));

  wrapCommand = { package, makeWrapper, ... }@args:
...
pkgs ? null, lib ? pkgs.lib, makeWrapper ? pkgs.makeWrapper, }:
let
  wrap = lib.throwIf ((package.buildCommandPath or "") != "")
    "wrapFirejail doesn't support derivations using `buildCommandPath`"
    (if (package.buildCommand or "") != "" then wrapCommand else wrapPhase);
